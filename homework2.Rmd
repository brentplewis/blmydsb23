...
# Create a custom vector mapping abbreviated month names to numbers
abbreviated_months <- c("Jan", "Feb", "Mar", "Apr", "May", "Jun", 
                        "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")

month_numbers <- 1:12  # Numeric values corresponding to the months

# Convert abbreviated month names to numbers in the dataframe
mass_shootings$month <- month_numbers[match(mass_shootings$month, abbreviated_months)]
...
ggplot(racial_breakdown, aes(x = month, fill = race)) +
...
fraud_likelihood <- card_fraud %>%
  summarise(likelihood = mean(is_fraud)) %>% 
  print()

# Generate a table summarizing the number and frequency of fraudulent transactions per year
fraud_summary <- transactions %>%
  group_by(trans_year) %>%
  summarise(num_fraud = sum(is_fraud), frequency = num_fraud / n())

# Print the likelihood of fraudulent transactions
print(fraud_likelihood)

# Print the table summarizing fraudulent transactions per year
print(fraud_summary)
